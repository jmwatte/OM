function Invoke-StageB-AlbumSelection {
    ...
    if ($FetchAlbums) {
        ...
        if ($null -eq $albumsForArtist -or $albumsForArtist.Count -eq 0) {
            ...
            $inputF = Read-Host "Enter '(b)ack', '(s)kip or (x)ip', 'id:<id>', ps/pq/pd/pm, or album name to filter, (?) help"
            switch -Regex ($inputF) {
                '^\?$' {
                    Show-OMHelp -Context 'StageB-NoResults'
                    return @{
                        NextStage             = 'B'
                        SelectedAlbum         = $null
                        UpdatedCache          = $CachedAlbums
    ...
    while ($true) {
        ...
        $inputF = Read-Host "Select album(s) [number], (b)ack, (n)ext, (pr)ev, (x)ip, id:<id>, (p)rovider, (f)indmode, cv/cvo/cs/ct, *, (ni), (?) help, or text:"
        [Console]::ForegroundColor = $originalColor
        
        switch -Regex ($inputF) {
            '^\?$' {
                Show-OMHelp -Context 'StageB-Select'
                continue
            }
            '^x(ip)?$' {
                return @{
