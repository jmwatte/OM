Describe 'Parse-AlbumSelectionChoice' {
    BeforeAll {
        $pCandidates = @()
        if ($PSScriptRoot) { $pCandidates += Join-Path $PSScriptRoot '..\Utils\Parse-AlbumSelectionChoice.ps1' }
        if ($MyInvocation.MyCommand.Path) { $pCandidates += Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Path) '..\Utils\Parse-AlbumSelectionChoice.ps1' }
        $pCandidates += Join-Path (Get-Location).Path 'Private\Utils\Parse-AlbumSelectionChoice.ps1'
